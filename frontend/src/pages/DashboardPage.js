        ...
        <div className="container dashboard-container">
            ...
            <div className="main-content">
                ...
                {fictionalDataMessage && (
                    <div className="data-mode-notification">
                        {fictionalDataMessage}
                    </div>
                )}
                ...
                {activeSection === 'inicio' && (
                    <>
                        ...
                        {/* Gráfico de Pizza para não-admin */}
                        {localStorage.getItem('isAdmin') !== 'true' && (
                            <div style={{
                                ...
                                <h3 style={{ textAlign: 'center', color: '#FFF', marginBottom: 16 }}>
                                    Consumo por Tipo de Dispositivo(Fictício)
                                </h3>
                                <Doughnut
                                    data={{
                                        labels: ['Sala', 'Câmera'],
                                ...
                                <div className="chart-wrapper" style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
                                    <div style={{ width: 200, height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        {localStorage.getItem('isAdmin') === 'true' ? (
                                            ...
                                            <Doughnut
                                                data={{
                                                    labels: ['TV', 'Luz 1', 'Ar', 'Luz 2'],
                                                    datasets: [{
                                                        data: [35, 25, 20, 20],
                                                        backgroundColor: ['#00bcd4', '#ff9800', '#8bc34a', '#e91e63'],
                                                        borderColor: ['#00838f', '#f57c00', '#558b2f', '#ad1457'],
                                                        borderWidth: 1,
                                                    }],
                                                }}
                                                options={{
                                    ...
                                    {/* Legendas */}
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexDirection: 'column', fontSize: 13 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
                    <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 6, background: '#00bcd4', marginRight: 4 }}></span>
                    <span style={{ color: '#FFF', fontWeight: 500 }}>Sala:</span>
                    <span style={{ color: '#FFF', fontWeight: 700 }}>
                      {localStorage.getItem('isAdmin') === 'true' ? (() => {
                          const data = getConsumptionByTypeData();
                          const total = data.datasets[0].data.reduce((a, b) => a + b, 0);
                          return total > 0 ? ((data.datasets[0].data[0] / total) * 100).toFixed(1) : '0.0';
                      })() : '60.0'}
                        %
                    </span>
                  </span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
                    <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 6, background: '#ff9800', marginRight: 4 }}></span>
                    <span style={{ color: '#FFF', fontWeight: 500 }}>Câmera:</span>
                    <span style={{ color: '#FFF', fontWeight: 700 }}>
                      {localStorage.getItem('isAdmin') === 'true' ? (() => {
                          const data = getConsumptionByTypeData();
                          const total = data.datasets[0].data.reduce((a, b) => a + b, 0);
                          return total > 0 ? ((data.datasets[0].data[1] / total) * 100).toFixed(1) : '0.0';
                      })() : '40.0'}
                        %
                    </span>
                  </span>
                    ...
                    <div className="energy-control-section">
                        ...
                        <div className="bottom-sections-grid">
                            <div className="bottom-card">
                                <h1 style={{ textAlign: 'center', color: '#FFF', marginBottom: '1.5rem', fontSize: '1.5rem' }}>
