...
function DashboardPage() {
    ...
    const handleScheduleSubmit = async(e) => {
        ...
        if (selectedDays.length === 0) {
            setScheduleMessage('Por favor, selecione pelo menos um dia para o agendamento.');
            setScheduleMessageColor('red');
            return;
        }
        if (time === '00:00') {
        ...
        const selectedTopics = devices
            .filter(d => selectedDevices.includes(d.name.toLowerCase()) ||
                (d.name.toLowerCase().includes('sala') && selectedDevices.includes('sala')) ||
                (d.name.toLowerCase().includes('câmera') && selectedDevices.includes('camera')))
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_ENDPOINTS.TASMOTA}/schedule-power-off`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    // Função para atualizar o estado de cada timer
    const updateTimerForm = (idx, changes) => {
        setTimerForms(prev => prev.map((form, i) => i === idx ? {...form, ...changes } : form));
    };

    // Função para submeter cada timer
    const handleTimerSubmit = idx => async(e) => {
        e.preventDefault();
        const form = timerForms[idx];
        if (form.selectedDevices.length === 0) {
            updateTimerForm(idx, { scheduleMessage: 'Por favor, selecione pelo menos um dispositivo.', scheduleMessageColor: 'red' });
            return;
        }
        if (form.selectedDays.length === 0) {
            updateTimerForm(idx, { scheduleMessage: 'Por favor, selecione pelo menos um dia para o agendamento.', scheduleMessageColor: 'red' });
            return;
        }
        if (form.time === '00:00') {
            updateTimerForm(idx, { scheduleMessage: 'Por favor, selecione um horário para o agendamento.', scheduleMessageColor: 'red' });
            return;
        }
        const selectedTopics = devices
            .filter(d => form.selectedDevices.includes(d.name.toLowerCase()) ||
                (d.name.toLowerCase().includes('sala') && form.selectedDevices.includes('sala')) ||
                (d.name.toLowerCase().includes('câmera') && form.selectedDevices.includes('camera')))
            .map(d => d.tasmotaTopic);
        const payload = {
            devices: selectedTopics,
            days: form.allDaysSelected ? [] : form.selectedDays,
            repeat: form.repeat,
            time: form.time,
            timerNumber: idx + 1
        };
        try {
            ...
            const data = await response.json();
            if (response.ok) {
                updateTimerForm(idx, { scheduleMessage: data.message || 'Agendamento criado com sucesso!', scheduleMessageColor: 'green' });
                fetchSchedules();
            } else {
                updateTimerForm(idx, { scheduleMessage: data.message || 'Erro ao criar agendamento.', scheduleMessageColor: 'red' });
            }
        } catch (error) {
            updateTimerForm(idx, { scheduleMessage: 'Erro de rede ao criar agendamento.', scheduleMessageColor: 'red' });
        }
                            ...
                            <div className="bottom-card">
                                ...
                                {initialTimers.map((timer, idx) => (
                                    <form
                                        ...
                                        onSubmit={handleTimerSubmit(idx)}
                                    >
                                        <h2
                                            style={{ color: '#00bcd4', fontSize: 18, marginBottom: 8 }}
                                        >
                                            Timer {timer}
                                        </h2>
                                        ...
                                        <div style={{ width: '100%' }}>
                                            ...
                                            <div
                                                className={`menu-box-dropdown${
                                                    timerForms[idx].deviceDropdownOpen ? ' open' : ''
                                                }`}
                                                tabIndex={0}
                                            >
                                                <button
                                                    ...
                                                    className={`menu-box-dropdown-btn${
                                                        timerForms[idx].selectedDevices.length > 0
                                                            ? ' selected'
                                                            : ''
                                                    }`}
                                                    onClick={() =>
                                                        updateTimerForm(idx, {
                                                            deviceDropdownOpen: !timerForms[idx].deviceDropdownOpen
                                                        })
                                                    }
                                                >
                                                    {timerForms[idx].selectedDevices.length === 0
                                                        ? 'Selecionar dispositivo(s)'
                                                        : timerForms[idx].selectedDevices.length ===
                                                        devices.length
                                                            ? 'Todos'
                                                            : timerForms[idx].selectedDevices
                                                                .map((dev) => dev.charAt(0).toUpperCase() + dev.slice(1))
                                                                .join(', ')}
                                                ...
                                                <div className="menu-box-dropdown-list">
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            if (
                                                                ...
                                                                updateTimerForm(idx, { selectedDevices: [] });
                                                            else
                                                                updateTimerForm(idx, {
                                                            ...
                                                            updateTimerForm(idx, { deviceDropdownOpen: true });
                                                        }}
                                                    >
                                                        Todos
                                                    </button>
                                                    {devices.map((device) => (
                                                        <button
                                                            ...
                                                            type="button"
                                                            className={`menu-box-option${
                                                                timerForms[idx].selectedDevices.includes(
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                updateTimerForm(idx, {
                                                                    selectedDevices: already
                                                                        ? timerForms[idx].selectedDevices.filter(
                                        ...
                                        <div style={{ width: '100%' }}>
                                            ...
                                            <div
                                                className={`menu-box-dropdown${
                                                    timerForms[idx].dropdownOpen ? ' open' : ''
                                                }`}
                                                tabIndex={0}
                                            >
                                                <button
                                                    type="button"
                                                    className={`menu-box-dropdown-btn${
                                                        timerForms[idx].selectedDays.length > 0
                                                            ? ' selected'
                                                            : ''
                                                    }`}
                                                    onClick={() =>
                                                        updateTimerForm(idx, {
                                                            dropdownOpen: !timerForms[idx].dropdownOpen
                                                        })
                                                    }
                                                >
                                                    {timerForms[idx].selectedDays.length === 0
                                                        ? 'Selecionar dias'
                                                        : timerForms[idx].selectedDays.length === 7
                                                            ? 'Todos os dias'
                                                            : timerForms[idx].selectedDays
                                                                .map((i) => weekDays[i])
                                                                .join(', ')}
                                                    <span style={{ marginLeft: 8, fontSize: 13 }}>▼</span>
                                                </button>
                                                <div className="menu-box-dropdown-list">
                                                    <button
                                                        type="button"
                                                        className={`menu-box-option${
                                                            timerForms[idx].allDaysSelected ? ' selected' : ''
                                                        }`}
                                                        onClick={() => {
                                                            updateTimerForm(idx, {
                                                                allDaysSelected: !timerForms[idx].allDaysSelected,
                                                                selectedDays: []
                                                            });
                                                            updateTimerForm(idx, { dropdownOpen: true });
                                                        }}
                                                    >
                                                        Todos os dias
                                                    </button>
                                                    {weekDays.map((day, i) => (
                                                        <button
                                                            ...
                                                            type="button"
                                                            className={`menu-box-option${
                                                                timerForms[idx].selectedDays.includes(i)
                                                                    ? ' selected'
                                                                    : ''
                                                            }`}
                                                            onClick={() => {
                                                                ...
                                                                updateTimerForm(idx, {
                                                                    selectedDays: already
                                                                        ? timerForms[idx].selectedDays.filter(
                                                                            (d) => d !== i
                                                                        )
                                                                        : [...timerForms[idx].selectedDays, i]
                                                                });
                                                                updateTimerForm(idx, { dropdownOpen: true });
                                                            }}
                                                        >
                                        ...
                                        >
                                            <button
                                                type="button"
                                                className={`menu-box-option${
                                                    timerForms[idx].repeat ? ' selected' : ''
                                                }`}
                                                onClick={() =>
                                                    updateTimerForm(idx, { repeat: !timerForms[idx].repeat })
                                                }
                                                style={{ minWidth: 160, fontSize: '0.95rem', padding: '7px 10px' }}
                                            >
                                        ...
                                        >
                                            ...
                                            <input
                                                type="time"
                                                value={timerForms[idx].time}
                                                onChange={(e) =>
                                                    updateTimerForm(idx, { time: e.target.value })
                                                }
                                        ...
                                        <button
                                            ...
                                            style={{
                                                marginTop: 6,
                                                background:
                                                    'linear-gradient(90deg, #00bcd4 60%, #4a4a7e 100%)',
                                                color: '#FFF',
                                                border: 'none',
                                                borderRadius: 8,
                                                padding: '10px 0',
                                            ...
                                            disabled={timerForms[idx].loading}
                                        >
                                            {timerForms[idx].loading ? 'Agendando...' : 'Agendar'}
                                        </button>
                                        {timerForms[idx].scheduleMessage && (
                                            <p
                                                style={{
                                                    color: timerForms[idx].scheduleMessageColor,
                                                    marginTop: 8,
                                                    textAlign: 'center'
                                                }}
                                            >
                                ...
                                {loadingSchedules ? (
                                    ...
                                    >
                                        {schedules.map((sch) => (
                                            ...
                                            >
                                                <strong>Dispositivos:</strong> {sch.deviceIds.join(', ')}
                                                <br />
                                                <strong>Dias:</strong>{' '}
                                                {sch.days.length === 7
                                                    ? 'Todos'
                    ...
                    <div className="main-content2">
                        ...
                        >
                            ...
                            <h4 style={{ color: '#e0e0e0', marginBottom: 24 }}>
                                ...
                                futuro de energia e receber dicas para um uso mais eficiente dos
                                seus eletrodomésticos. Fique à vontade!
                            </h4>
                        </div>
                        <div
                            style={{
