    ...
    return ( <
        ...
        { /* ========== ENERGY CONTROL SECTION ========== */ } {
            ...
            div className = "bottom-sections-grid" >
                ...
                /h1> <
            form style = {
                { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, width: '100%' }
            }
            onSubmit = { handleScheduleSubmit } > { /* Seleção de Dispositivo */ } <
                div style = {
                    { width: '100%', marginBottom: 8 }
                } >
                <
                label style = {
                    { color: '#FFF', fontWeight: 500, marginBottom: 8, display: 'block', textAlign: 'center', fontSize: 16 }
                } > Dispositivo: < /label> <
            div style = {
                ...
                <
                input type = "checkbox"
            checked = { selectedDevices.includes('sala') }
            onChange = {
                () => handleDeviceSelect('sala')
            }
            /> <
            span className = "checkmark" > < /span> Sonoff Sala < /
            label > <
                label className = "custom-checkbox" >
                <
                input type = "checkbox"
            checked = { selectedDevices.includes('camera') }
            onChange = {
                () => handleDeviceSelect('camera')
            }
            /> <
            span className = "checkmark" > < /span> Sonoff Câmera < /
            label > <
                label className = "custom-checkbox" >
                <
                input type = "checkbox"
            checked = { selectedDevices.length === 2 }
            onChange = {
                () => {
            ...
            /> <
            span className = "checkmark" > < /span> Ambos < /
            label > <
                /div> < /
            div > { /* Seleção de Dias */ } <
                div style = {
            ...
            onChange = { handleAllDaysSelect }
            /> <
            span className = "checkmark" > < /span> Todos os dias < /
            label > {
                    weekDays.map((day, idx) => ( <
                        ...
                        /> <
                        span className = "checkmark" > < /span> {day} < /
                        label >
                    ))
                } <
                /div> < /
            ...
            /> <
            span className = "checkmark" > < /span> Repetir nos dias selecionados < /
            label > <
                /div> { / * Horário * / } <
            div style = {
                    { width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 8 }
                } >
                <
                label style = {
                    { color: '#FFF', fontWeight: 500, marginBottom: 4, fontSize: 16 }
                } > Horário: < /label> <
            input type = "time"
            value = { time }
            onChange = { e => setTime(e.target.value) }
            style = {
                { width: 120, fontSize: 16, borderRadius: 6, border: '1px solid #4a4a7e', padding: 6, background: '#23243a', color: '#FFF', textAlign: 'center' }
            }
            required / >
                <
                /div> { / * Botão * / } <
            button type = "submit"
            style = {
                    { marginTop: 8, background: 'linear-gradient(90deg, #00bcd4 60%, #4a4a7e 100%)', color: '#FFF', border: 'none', borderRadius: 8, padding: '12px 0', fontWeight: 'bold', fontSize: 18, cursor: 'pointer', width: '70%' }
                } >
                Agendar <
                /button> {
            scheduleMessage && < p style = {
                { color: scheduleMessageColor, marginTop: 8, textAlign: 'center' }
            } > { scheduleMessage } < /p>} < /
            form > <
                /div>
