    ...
    return ( <
        ...
        { /* Seção "Início" */ } {
            activeSection === 'inicio' && ( <
                ...
                p >
                R$ { ' ' } {
                    (parseFloat(currentMonthConsumption.replace(' kWh', '')) * 0.75).toFixed(
                        2
                    )
                } <
                /p> < /
                ...
                h3 > Consumo total < /h3> <
                p > {
                    devices.length > 0 && devices[0].powerState && devices[0].latestReading && typeof devices[0].latestReading.totalEnergy === 'number' ?
                    ...
                    devices.length > 0 ? ( <
                        div className = "device-control-list" > {
                            devices.map((device) => ( <
                                ...
                                type = "button"
                                style = {
                                    { cursor: device.powerState ? 'pointer' : 'not-allowed' }
                                }
                                ...
                                className = { `device-toggle-button ${!device.powerState ? 'off' : 'on'}` }
                                type = "button"
                                style = {
                                    { cursor: !device.powerState ? 'pointer' : 'not-allowed', marginLeft: 8 }
                                }
                                disabled = { device.powerState } >
                                Ligar <
                                /button> < /
