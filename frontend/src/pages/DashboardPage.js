...
function DashboardPage() {
    ...
    // NOVO: Estado para indicar se o usuário é admin
    ...
    useEffect(() => {
        ...
        setIsAdmin(storedIsAdmin === 'true' || storedIsAdmin === true);
    ...
    return ( <
        ...
        div className = "main-content" > {!isAdmin && ( <
                div style = {
                    {
                ...
                Os dados exibidos são fictícios porque sua conta não possui permissões de administrador.Entre em contato com o suporte para solicitar acesso como administrador e visualizar os dados reais. <
                /div>
            )
        }
                ...
                div className = "device-report-list" > {
                    report.details.length > 0 ? (
                        report.details.map((detail, index) => ( <
                                ...
                                p >
                                Status Atual: { ' ' } <
                                span className = { devices[index] && devices[index].powerState ? 'status-on-text' : 'status-off-text' } > { devices[index] && devices[index].powerState ? 'Ligado' : 'Desligado' } <
                                /span> < /
                                p > <
